{ self, inputs, ... }: {
  ...
  ## NFS server -- mirrors rpi-ar/services/nfs.nix. nas-ar is meant to
  ## replace rpi-ar, so it reuses the same client allowlist and export
  ## layout so existing NFS clients (mbpro, etc.) need no changes beyond
  ## pointing at nas-ar's IP.
  flake.nixosModules.nasArNfs = { config, lib, pkgs, ... }:
    let
